{
  flake.nixosModules.fireshare =
    ...
    {
      options.self.fireshare = {
        enable = mkEnableOption "fireshare";

        package = mkPackageOption pkgs "fireshare" { };

        backendListenAddress = mkOption {
          type = types.str;
        ...
        dataDir = mkOption {
          type = types.path;
          default = "/var/lib/fireshare";
        };

        enableWrappedCli =
          (mkEnableOption "the fireshare cli wrapped with necessary environment variables set")
          // {
            default = true;
          };
